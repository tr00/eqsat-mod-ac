...
public:
    ...
    ~AbstractSet() {
        switch(kind) {
            case SORTED_VEC: sorted_vec.~SortedVecSet(); break;
            case SORTED_ITER: sorted_iter.~SortedIterSet(); break;
        }
    }
    ...
    AbstractSet(AbstractSet&& other) : kind(other.kind) {
        switch (kind) {
            case SORTED_VEC: new(&sorted_vec) SortedVecSet(std::move(other.sorted_vec)); break;
            case SORTED_ITER: new(&sorted_iter) SortedIterSet(std::move(other.sorted_iter)); break;
        }
    }
    ...
    bool contains(id_t id) const {
        switch(kind) {
            case SORTED_VEC: return sorted_vec.contains(id);
            case SORTED_ITER: return sorted_iter.contains(id);
        }
        assert(0);
    }
    ...
    size_t size() const {
        switch(kind) {
            case SORTED_VEC: return sorted_vec.size();
            case SORTED_ITER: return sorted_iter.size();
        }
        assert(0);
    }
    ...
    void for_each(Func f) const {
        switch(kind) {
            case SORTED_VEC: return sorted_vec.for_each(f);
            case SORTED_ITER: return sorted_iter.for_each(f);
        }
        assert(0);
    }
};
