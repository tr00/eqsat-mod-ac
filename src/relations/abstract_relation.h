...
public:
    ...
    ~AbstractRelation() {
        switch (kind) {
            case ROW_STORE: row_store.~RowStore(); break;
        }
    }
    ...
    AbstractRelation(AbstractRelation&& other) : kind(other.kind) {
        switch (kind) {
            case ROW_STORE: new(&row_store) RowStore(std::move(other.row_store)); break;
        }
    }
    ...
    symbol_t get_operator_symbol() const {
        switch (kind) {
            case ROW_STORE: return row_store.get_operator_symbol();
        }
        assert(0);
    }
    ...
    size_t size() const {
        switch (kind) {
            case ROW_STORE: return row_store.size();
        }
        assert(0);
    }
    ...
    void add_tuple(const std::vector<id_t>& tuple) {
        switch (kind) {
            case ROW_STORE: return row_store.add_tuple(tuple);
        }
        assert(0);
    }
    ...
    AbstractIndex build_index(uint32_t vo) {
        switch (kind) {
            case ROW_STORE: return row_store.build_index(vo);
        }
        assert(0);
    }
