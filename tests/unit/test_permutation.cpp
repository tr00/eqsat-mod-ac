...
TEST_CASE("index_to_permutation basic cases", "[permutation]")
{
    SECTION("Empty elements")
    {
        ...
        REQUIRE(index_to_permutation(0, elements) == Vec<uint32_t>{});
        REQUIRE_THROWS_AS(index_to_permutation(1, elements), std::invalid_argument);
    }

    SECTION("Single element")
    {
        Vec<uint32_t> elements = {5};
        REQUIRE(index_to_permutation(0, elements) == Vec<uint32_t>{5});
        REQUIRE_THROWS_AS(index_to_permutation(1, elements), std::invalid_argument);
    }

    SECTION("Two elements")
    {
        Vec<uint32_t> elements = {0, 1};
        REQUIRE(index_to_permutation(0, elements) == Vec<uint32_t>{0, 1});
        REQUIRE(index_to_permutation(1, elements) == Vec<uint32_t>{1, 0});
    ...
    SECTION("Three elements - all indices")
    {
        ...
        REQUIRE(index_to_permutation(0, elements) == Vec<uint32_t>{0, 1, 2});
        REQUIRE(index_to_permutation(1, elements) == Vec<uint32_t>{0, 2, 1});
        REQUIRE(index_to_permutation(2, elements) == Vec<uint32_t>{1, 0, 2});
        REQUIRE(index_to_permutation(3, elements) == Vec<uint32_t>{1, 2, 0});
        REQUIRE(index_to_permutation(4, elements) == Vec<uint32_t>{2, 0, 1});
        REQUIRE(index_to_permutation(5, elements) == Vec<uint32_t>{2, 1, 0});
...
TEST_CASE("Round-trip conversions", "[permutation]")
{
    SECTION("All permutations of size 3")
    {
        ...
        Vec<Vec<uint32_t>> all_perms = {{0, 1, 2}, {0, 2, 1}, {1, 0, 2}, {1, 2, 0}, {2, 0, 1}, {2, 1, 0}};

        for (size_t i = 0; i < all_perms.size(); ++i)
        {
            // perm -> index -> perm
            uint32_t index = permutation_to_index(all_perms[i]);
            Vec<uint32_t> recovered = index_to_permutation(index, elements);
            REQUIRE(recovered == all_perms[i]);
            REQUIRE(index == i);

            // index -> perm -> index
            Vec<uint32_t> perm = index_to_permutation(i, elements);
...
TEST_CASE("apply_permutation with precomputed indices", "[permutation]")
{
    ...
    {
        Vec<uint32_t> perm_indices = {2, 0, 1}; // [2,0,1] permutation
        Vec<uint32_t> vec = {10, 20, 30};
        apply_permutation(perm_indices, vec);
        REQUIRE(vec == Vec<uint32_t>{30, 10, 20});
    }

    SECTION("Identity permutation")
    {
        Vec<uint32_t> perm_indices = {0, 1, 2};
        Vec<uint32_t> vec = {10, 20, 30};
        apply_permutation(perm_indices, vec);
        REQUIRE(vec == Vec<uint32_t>{10, 20, 30});
    }

    SECTION("Reverse permutation")
    {
        Vec<uint32_t> perm_indices = {2, 1, 0};
        Vec<uint32_t> vec = {10, 20, 30};
        apply_permutation(perm_indices, vec);
        REQUIRE(vec == Vec<uint32_t>{30, 20, 10});
    }

    SECTION("Empty vectors")
    {
        Vec<uint32_t> perm_indices = {};
        Vec<uint32_t> vec = {};
        apply_permutation(perm_indices, vec);
        REQUIRE(vec.empty());
    }
    ...
    SECTION("Single element")
    {
        Vec<uint32_t> perm_indices = {0};
        Vec<uint32_t> vec = {42};
        apply_permutation(perm_indices, vec);
        REQUIRE(vec == Vec<uint32_t>{42});
    }
}
...
TEST_CASE("apply_permutation with precomputed indices - error handling", "[permutation]")
{
    ...
    {
        Vec<uint32_t> perm_indices = {0, 1};
        Vec<uint32_t> vec = {10, 20, 30};
        REQUIRE_THROWS_AS(apply_permutation(perm_indices, vec), std::invalid_argument);

        Vec<uint32_t> perm_indices2 = {0, 1, 2};
        Vec<uint32_t> vec2 = {10, 20};
        REQUIRE_THROWS_AS(apply_permutation(perm_indices2, vec2), std::invalid_argument);
    }

    SECTION("Index out of bounds")
    {
        Vec<uint32_t> perm_indices = {0, 3, 1}; // 3 is out of bounds for size 3
        Vec<uint32_t> vec = {10, 20, 30};
        REQUIRE_THROWS_AS(apply_permutation(perm_indices, vec), std::out_of_range);

        Vec<uint32_t> perm_indices2 = {0, 1, 5}; // 5 is out of bounds
        Vec<uint32_t> vec2 = {10, 20, 30};
...
TEST_CASE("apply_permutation performance comparison setup", "[permutation]")
{
    ...
    {
        Vec<uint32_t> original = {10, 20, 30};

        // Precompute permutation indices once
        Vec<uint32_t> indices = {0, 1, 2};
        Vec<uint32_t> perm_indices = index_to_permutation(4, indices); // [2,0,1]

        // Apply to multiple vectors
        for (uint32_t i = 0; i < 5; ++i)
        {
            Vec<uint32_t> vec = {10 + i, 20 + i, 30 + i};
            apply_permutation(perm_indices, vec);
            REQUIRE(vec == Vec<uint32_t>{30 + i, 10 + i, 20 + i});
        }
    }
}
...
TEST_CASE("Non-zero based permutations", "[permutation]")
{
    ...
    {
        Vec<uint32_t> perm1 = {1, 2, 3};
        Vec<uint32_t> perm2 = {3, 1, 2};
        ...
        // Round-trip test
        Vec<uint32_t> elements = {1, 2, 3};
        Vec<uint32_t> recovered1 = index_to_permutation(index1, elements);
        Vec<uint32_t> recovered2 = index_to_permutation(index2, elements);
    ...
    {
        Vec<uint32_t> elements = {5, 6, 7};

        // Test a few permutations
        Vec<uint32_t> perm1 = {5, 6, 7}; // index 0
        Vec<uint32_t> perm2 = {7, 5, 6}; // index 4
...
TEST_CASE("Error handling", "[permutation]")
{
    ...
    SECTION("index_to_permutation errors")
    {
        ...
        Vec<uint32_t> large_elements(13);
...
TEST_CASE("Performance test with larger permutations", "[permutation]")
{
    ...
    {
        Vec<uint32_t> elements = {0, 1, 2, 3, 4, 5};

        // Test first and last permutations
        Vec<uint32_t> first_perm = index_to_permutation(0, elements);
        Vec<uint32_t> last_perm = index_to_permutation(719, elements); // 6! - 1

        REQUIRE(first_perm == Vec<uint32_t>{0, 1, 2, 3, 4, 5});
        REQUIRE(last_perm == Vec<uint32_t>{5, 4, 3, 2, 1, 0});
...
TEST_CASE("apply_permutation basic cases", "[permutation]")
{
    ...
    SECTION("Single element")
    {
        ...
        Vec<uint32_t> vec2 = {42};
        REQUIRE_THROWS_AS(apply_permutation(1, vec2), std::invalid_argument);
    }

    SECTION("Two elements")
    {
        Vec<uint32_t> vec1 = {10, 20};
        apply_permutation(0, vec1);
        REQUIRE(vec1 == Vec<uint32_t>{10, 20});

        Vec<uint32_t> vec2 = {10, 20};
        apply_permutation(1, vec2);
        REQUIRE(vec2 == Vec<uint32_t>{20, 10});

        Vec<uint32_t> vec3 = {10, 20};
    ...
    SECTION("Three elements - all permutations")
    {
        ...
        Vec<uint32_t> vec0 = {100, 200, 300};
        apply_permutation(0, vec0);
        REQUIRE(vec0 == Vec<uint32_t>{100, 200, 300});

        // Index 1: [0,2,1] -> [100, 300, 200]
        Vec<uint32_t> vec1 = {100, 200, 300};
        apply_permutation(1, vec1);
        REQUIRE(vec1 == Vec<uint32_t>{100, 300, 200});

        // Index 2: [1,0,2] -> [200, 100, 300]
        Vec<uint32_t> vec2 = {100, 200, 300};
        apply_permutation(2, vec2);
        REQUIRE(vec2 == Vec<uint32_t>{200, 100, 300});

        // Index 3: [1,2,0] -> [200, 300, 100]
        Vec<uint32_t> vec3 = {100, 200, 300};
        apply_permutation(3, vec3);
        REQUIRE(vec3 == Vec<uint32_t>{200, 300, 100});

        // Index 4: [2,0,1] -> [300, 100, 200]
        Vec<uint32_t> vec4 = {100, 200, 300};
        apply_permutation(4, vec4);
        REQUIRE(vec4 == Vec<uint32_t>{300, 100, 200});

        // Index 5: [2,1,0] -> [300, 200, 100]
        Vec<uint32_t> vec5 = {100, 200, 300};
        apply_permutation(5, vec5);
        REQUIRE(vec5 == Vec<uint32_t>{300, 200, 100});

        Vec<uint32_t> vec6 = {100, 200, 300};
...
TEST_CASE("apply_permutation consistency with index_to_permutation", "[permutation]")
{
    SECTION("Four elements - verify consistency")
    {
        ...
        for (uint32_t perm_index = 0; perm_index < 24; ++perm_index)
        {
            ...
            Vec<uint32_t> perm_indices = index_to_permutation(perm_index, indices);

            // Apply it manually
            Vec<uint32_t> expected;
...
TEST_CASE("apply_permutation with duplicate values", "[permutation]")
{
    ...
    {
        Vec<uint32_t> vec = {5, 5, 5};

        // All permutations should give the same result since all elements are identical
        for (uint32_t i = 0; i < 6; ++i)
        {
            Vec<uint32_t> test_vec = vec;
            apply_permutation(i, test_vec);
            REQUIRE(test_vec == Vec<uint32_t>{5, 5, 5});
        }
    }

    SECTION("Vector with some duplicates")
    {
        Vec<uint32_t> vec = {1, 2, 1};

        Vec<uint32_t> vec0 = vec;
        apply_permutation(0, vec0);
        REQUIRE(vec0 == Vec<uint32_t>{1, 2, 1}); // [0,1,2]
        Vec<uint32_t> vec1 = vec;
        apply_permutation(1, vec1);
        REQUIRE(vec1 == Vec<uint32_t>{1, 1, 2}); // [0,2,1]
        Vec<uint32_t> vec2 = vec;
        apply_permutation(2, vec2);
        REQUIRE(vec2 == Vec<uint32_t>{2, 1, 1}); // [1,0,2]
        Vec<uint32_t> vec3 = vec;
        apply_permutation(3, vec3);
        REQUIRE(vec3 == Vec<uint32_t>{2, 1, 1}); // [1,2,0]
        Vec<uint32_t> vec4 = vec;
        apply_permutation(4, vec4);
        REQUIRE(vec4 == Vec<uint32_t>{1, 1, 2}); // [2,0,1]
        Vec<uint32_t> vec5 = vec;
        apply_permutation(5, vec5);
        REQUIRE(vec5 == Vec<uint32_t>{1, 2, 1}); // [2,1,0]
    }
}
