...
TEST_CASE("TrieIndex navigation", "[trie_index]") {
    ...
    SECTION("Basic select operation") {
        ...
        REQUIRE(index.select(10) == true);
        REQUIRE(index.select(5) == false);   // Key doesn't exist
        REQUIRE(index.select(15) == false);  // Can't select after moving to child
    }
    ...
    SECTION("Project after select") {
        ...
        // Initially at root
        std::vector<id_t> keys = index.project();
        REQUIRE(keys.size() == 2);
        REQUIRE(keys[0] == 10);
        REQUIRE(keys[1] == 15);
    ...
    SECTION("Select and backtrack") {
        ...
        // Navigate down
        REQUIRE(index.select(10) == true);
        std::vector<id_t> child_keys = index.project();
        REQUIRE(child_keys.size() == 2);
        
        // Backtrack to root
        REQUIRE(index.backtrack() == true);
        std::vector<id_t> root_keys = index.project();
        REQUIRE(root_keys.size() == 2);
        REQUIRE(root_keys[0] == 10);
        REQUIRE(root_keys[1] == 15);
    }
    ...
    SECTION("Multiple select operations") {
        ...
        REQUIRE(index.select(10) == true);
        REQUIRE(index.select(20) == true);
        
        // Should be at leaf node
        std::vector<id_t> leaf_keys = index.project();
